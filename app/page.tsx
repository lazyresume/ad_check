...
const plans: Record<Plan, { label: string; title: string; price: string; desc: string; items: string[] }> = {
  ai: { label: "AI 전화", title: "커리어 AI", price: "29,000원 / 월", desc: "부담 없는 AI가 매달 당신의 일을 묻습니다.", items: ["월 1회 20분 AI 전화", "이력서·경력기술서 업데이트", "PDF 이메일 발송"] },
  human: { label: "전문가 전화", title: "커리어 전문가", price: "59,000원 / 월", desc: "10년 차 커리어 전문가가 성장을 정리합니다.", items: ["월 1회 30분 전문가 전화", "이력서·경력기술서·포트폴리오", "다음 달 커리어 액션 플랜"] },
};
...
export default function Home() {
  ...
  return <main>
    ...
    <section className="scene hero-scene" id="top"><div className="scene-inner hero-inner"><p className="tiny-label">LAZYRESUME / MONTHLY CAREER RECORD</p><h1>이번 달의 나를,<br /><strong>전화 한 통으로</strong><br />남겨보세요.</h1><p className="scene-sub">정리하지 않아도 괜찮아요.<br />이번 달에 있었던 일을 편하게 말하면<br />당신의 경력이 문서가 됩니다.</p><a className="black-pill" href="#story">어떻게 하는지 보기 <span>↓</span></a></div><div className="hero-art"><div className="phone"><div className="phone-top">게으른 이력서 <span>●●●</span></div><div className="phone-wave"><i /><i /><i /><i /><i /></div><p>이번 달에<br /><b>가장 뿌듯했던 일은?</b></p><small>00:20:00</small><div className="phone-button">●</div></div></div></section>
    <section className="scene statement-scene"><div className="scene-inner statement reveal"><p className="tiny-label">THE PROBLEM</p><h2>경력은 매달 쌓이는데<br /><span>이력서는 이직할 때만</span><br />열어보니까요.</h2><p>그때가 되면 작은 성취는 사라지고,<br />빈 문서 앞에서 다시 시작합니다.</p></div></section>
    <section className="scene story-scene" id="story"><div className="scene-inner story-inner reveal"><p className="tiny-label">HOW IT WORKS</p><h2>당신은 말하기만 하세요.<br /><strong>정리는 우리가 할게요.</strong></h2><div className="story-steps"><div><b>01</b><h3>말하기</h3><p>“이번 달에는 이런 일을 했어요.”<br />정리하지 않고 그대로 말해요.</p></div><div><b>02</b><h3>발견하기</h3><p>대화 속에 숨은 성과와<br />다음 이야기를 찾아요.</p></div><div><b>03</b><h3>남기기</h3><p>이력서와 경력기술서에<br />오늘의 나를 기록해요.</p></div></div></div></section>
